...
public class Main extends Application {
    ...
    private static List<Double> loadData() {
        ...
        LineMapper lineMapper = new ObjectLineMapper.ObjectLineMapperBuilder(MonthlyData.class).prepare(prepareFunction).build(); 

        List<MonthlyData> results = null;
        CSVReader csvreader = null;
        try {
            csvreader = new CSVReader.CSVReaderBuilder(filePath, MonthlyData.class).withMapper(lineMapper).build();
        ...
        for (MonthlyData monthlyData : results.subList(0, 12)) {
            averages.add
              (monthlyData.getDays().stream()
                  .filter(day -> "V".equals(day.getControl()))
